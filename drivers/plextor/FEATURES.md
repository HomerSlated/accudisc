...
| 6 | **SilentMode** (speed/noise caps) | `0xE9` MODE, pages `0x06`/`0x07`/`0x08` | ☑ | ◐ | **Page `0x08` GET-verified** — full settings block (`08 06 00 04 08 00 19 0d`), `resp[2]=0` so Silent Mode is currently **OFF**. **Pages `0x06`/`0x07` are NOT GET-verified: measured 2026-09-09 they return `4/00/00` + DID_ERROR with nothing transferred** (`OPCODES.md` §I). One page was verified and three carried the tick. Most likely feature-gated — sub-pages of a feature that is off — and the discriminator is a SET of page `0x08` followed by a re-read, which no phase has run. Disc=0x06, Tray=0x07, Main=0x08 remains the PlexTools mapping, not a response we have seen. |
