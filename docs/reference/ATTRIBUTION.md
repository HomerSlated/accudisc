...
- **Sanyo Semiconductor's 2005 DVD/CD LSI product brochure** (originally
  `ftpmirror.your.org/pub/misc/bitsavers/components/sanyo/cdrom/2005_Sanyo_DVD_CD_LSI_Brochure.pdf`,
  fetched via the Wayback Machine, snapshot 20240801155600, since the origin
  refused a direct fetch) — Sanyo's own published description of the
  LC897491/LC897492 signal-processing LSIs, direct siblings of the PX-716A's
  LC897496K, stating the embedded core is a **80251** ("Built-in CPU(80251)"
  / "Source code compatible with the Intel 8051, full compatibility with the
  Intel 80251"). A public vendor product brochure, not a licensed spec; no
  content is redistributed here beyond the two quoted phrases needed to
  identify the fact. Used in `drivers/plextor/PROTOCOL.md` to identify the
  CPU architecture of `rome_111.bin`, cross-checked empirically against the
  firmware image with `rz-ghidra`'s SLEIGH `80251:BE:24:default` decoder.
