 ...
 * ("Not Ready, long write in progress": SK 2 / ASC 04 / ASCQ 08). */
static int write_chunk(struct accudisc_device *dev, int32_t lba,
                       const uint8_t *buf, uint32_t nsec)
{
    for (;;) {
        int rc = adsc_mmc_write10(dev, lba, nsec, buf, SECTOR);
        if (rc == ACCUDISC_OK)
            return ACCUDISC_OK;
                   ...
                   adsc_burn_progress cb, void *user)
{
    ...
    /* 1. Program DAO write parameters (test-write in simulate). */
    ...
    for (uint32_t left = LEADIN_GAP; left > 0;) {
        ...
        if ((ret = write_chunk(dev, lba, zero, n)) != ACCUDISC_OK)
            goto done;
        lba += (int32_t)n;
        left -= n;
    }
    ...
    for (int i = 0; i < toc->ntracks; i++) {
        ...
        for (uint32_t rem = t->sectors; rem > 0;) {
            ...
            if ((ret = write_chunk(dev, lba, chunk, n)) != ACCUDISC_OK)
                goto done;
            lba += (int32_t)n;
